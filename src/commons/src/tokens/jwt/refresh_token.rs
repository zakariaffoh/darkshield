...
use super::{
    access_token::AccessTokenImp, id_token::IdTokenImp, json_web_token::JsonWebToken, AccessToken,
    IdToken, Jwt, RefreshToken,
};
...
impl RefreshTokenImp {
    ...
    pub fn set_audience(&mut self, audience: impl Into<String>) {
        ...
        self.claims.insert(key.clone(), Value::String(value));
    }
    ...
    pub fn set_realm_access(&mut self, access: super::AccessTokenAccess) {
        RefreshTokenImp::set_access_to_map("realm-access".to_string(), access, &mut self.claims);
    }

    pub fn set_resources_access(&mut self, accesses: BTreeMap<String, super::AccessTokenAccess>) {
        let mut access_map = Map::new();
        for (access_key, access) in accesses {
            RefreshTokenImp::set_access_to_map(access_key, access, &mut access_map);
        }
        self.claims
    ...
    fn check_claim(key: &str, value: &Value) -> Result<(), JwtError> {
        (|| -> anyhow::Result<()> {
            match key {
                ...
                | "claims_locales" | "acr" | "s_hash" | "scope" | "aud" => match &value {
                    Value::String(_) => {}
...
impl AsRef<Map<String, Value>> for RefreshTokenImp {
    fn as_ref(&self) -> &Map<String, Value> {
        &self.claims
    }
}

impl Into<Map<String, Value>> for RefreshTokenImp {
    fn into(self) -> Map<String, Value> {
        self.into_map()
    }
}

impl Display for RefreshTokenImp {
...
impl TryInto<IdTokenImp> for RefreshTokenImp {
    type Error = JwtError;
...
impl TryInto<AccessTokenImp> for RefreshTokenImp {
    type Error = JwtError;

    fn try_into(self) -> Result<AccessTokenImp, Self::Error> {
        AccessTokenImp::from_map(self.into_map())
    }
}

impl TryInto<JsonWebToken> for RefreshTokenImp {
    type Error = JwtError;
