...
impl JweContext {
    pub fn new() -> Self {
        Self {
            ...
            compressions: {
                let compressions: Vec<Box<dyn JweCompression>> =
                    vec![Box::new(DeflateJweCompression::Def)];
        ...
        F: Fn(usize, &JweHeader) -> Option<&'a dyn JweEncrypter>,
    {
        (|| -> anyhow::Result<String> {
            ...
            for (i, recipient_header) in recipient_headers.iter().enumerate() {
                ...
                if let None = merged.kid() {
                    if let Some(kid) = encrypter.kid() {
                        recipient_header.set_kid(kid.to_string());
                    }
                }
        ...
        F: Fn(&JweHeader) -> Option<&'a dyn JweEncrypter>,
    {
        (|| -> anyhow::Result<String> {
            ...
            if let None = merged.kid() {
                if let Some(kid) = encrypter.kid() {
                    protected.set_kid(kid.to_string());
                }
            }
        ...
        F: Fn(&JweHeader) -> Result<Option<&'a dyn JweDecrypter>, JoseError>,
    {
        (|| -> anyhow::Result<(Vec<u8>, JweHeader)> {
            ...
            match decrypter.kid() {
                Some(expected) => match merged.kid() {
    ...
    pub fn deserialize_json<'a>(
        ...
        self.deserialize_json_with_selector(input, |header| {
            ...
            match decrypter.kid() {
                Some(expected) => match header.kid() {
                    Some(actual) if expected == actual => {}
                    _ => return Ok(None),
                },
                None => {}
...
mod tests {
    ...
    fn compact_dir() -> Result<()> {
        let jwe_context: JweContext = JweContext::new();
        ...
        for (cipher, keylen) in CONTENT_CIPHERS {
            ...
            let jwe = jwe_context.serialize_compact(payload, &header, &encrypter)?;
            println!("{}", jwe);

            let decrypter = alg.decrypter_from_bytes(&key)?;
            let (data, _header) = jwe_context.deserialize_compact(&jwe, &decrypter)?;
    ...
    fn flattened_json_dir() -> Result<()> {
        ...
        for (cipher, keylen) in CONTENT_CIPHERS {
            ...
            let encrypter = alg.encrypter_from_bytes(&key)?;
            let jwe =
                jwe_context.serialize_flattened_json(payload, Some(&hs), None, None, &encrypter)?;
            println!("{}", jwe);

            let decrypter = alg.decrypter_from_bytes(&key)?;
            let (data, _header) = jwe_context.deserialize_json(&jwe, &decrypter)?;
            assert_eq!(data, payload);
        }
        Ok(())
    }
    ...
    fn general_json_dir() -> Result<()> {
        ...
        for (cipher, keylen) in CONTENT_CIPHERS {
            ...
            let key = vec![0; keylen];
            let encrypter = alg.encrypter_from_bytes(&key)?;
            let jwe = jwe_context.serialize_general_json(
                payload,
                Some(&hs),
                &[(None, &encrypter)],
                None,
